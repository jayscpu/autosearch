#!/usr/bin/env python3
"""
Hyperparameter sweep for clip_sm (h256/L2, 65 features, seed 42).

Grid (300 configs):
  window  ∈ {15, 20, 30, 40, 60}
  horizon ∈ {12, 18, 30, 42, 60}     (all multiples of sub_window=6)
  lr      ∈ {5e-4, 1e-3, 3e-3}
  dropout ∈ {0.2, 0.3, 0.4, 0.5}

sub_window = 6 (matches baseline clip_sm). All horizons divide evenly:
  n_steps = 2, 3, 5, 7, 10 for horizon = 12, 18, 30, 42, 60.

Logs one row per config to hyperparam_sweep/sweep_results.tsv, written
incrementally so partial progress is recoverable if interrupted.
"""
...
BASE = {
    ...
    "sub_window": 6,  # matches baseline clip_sm
    "train_stride_ratio": 0.5,  # train_stride = window * ratio (matches original 30→15)
    "warmup_frames": 400,
    "train_intersections": [
...
GRID = {
    "window": [15, 20, 30, 40, 60],
    "horizon": [12, 18, 30, 42, 60],
    "lr": [5e-4, 1e-3, 3e-3],
    "dropout": [0.2, 0.3, 0.4, 0.5],
}

TSV_PATH = SCRIPT_DIR / "sweep_results.tsv"
HEARTBEAT_PATH = SCRIPT_DIR / "sweep_heartbeat.txt"


def _fsync_write(path: Path, mode: str, content: str) -> None:
    """Append/write content and force it to disk (flush + fsync)."""
...
TSV_COLUMNS = [
    ...
    "n_steps", "train_stride", "eval_stride",
    "r_nano", "r_ns", "r_sm", "combined_score",
    "best_val_loss", "best_epoch", "elapsed_s",
]
...
def _write_heartbeat(label, epoch, val_loss, best, pat, config_idx, n_configs):
    content = (f"config {config_idx}/{n_configs}  {label}\n"
               f"epoch={epoch}  val_loss={val_loss:.6f}  "
               f"best={best:.6f}  patience={pat}\n"
               f"timestamp={time.strftime('%Y-%m-%d %H:%M:%S')}\n")
    _fsync_write(HEARTBEAT_PATH, "w", content)


def train_one(X_train, y_train, X_val, y_val, n_feat, n_steps,
              lr, dropout, device, label, config_idx, n_configs):
    ...
    for epoch in range(BASE["max_epochs"]):
        ...
        if vloss < best:
            ...
            pat += 1
        # Heartbeat every 5 epochs — fsync'd so it's visible if we crash
        if epoch % 5 == 0:
            _write_heartbeat(label, epoch, vloss, best, pat, config_idx, n_configs)
...
def main():
    ...
    if TSV_PATH.exists():
        try:
            ...
            print(f"  Could not read existing TSV ({e}); starting fresh", flush=True)
    else:
        _fsync_write(TSV_PATH, "w", "\t".join(TSV_COLUMNS) + "\n")

    configs = list(itertools.product(
        GRID["window"], GRID["horizon"], GRID["lr"], GRID["dropout"]))
    print(f"  Total configs: {len(configs)}", flush=True)

    # Cache windows by (window, horizon) so we don't re-window 240 times.
    # Train stride = window // 2, eval stride = window (approx).
    window_cache = {}

    for ci, (window, horizon, lr, dropout) in enumerate(configs):
        ...
        key = (window, horizon, train_stride, eval_stride)
        if key not in window_cache:
            ...
            window_cache[key] = (X_train, y_train, X_es, y_es,
                                 X_eval, y_eval_mean)
            print(f"  [cache] built windows {key}: "
                  f"train={len(y_train)} es={len(y_es)} eval={len(y_eval)}",
                  flush=True)

        X_train, y_train, X_es, y_es, X_eval, y_eval_mean = window_cache[key]
        ...
        try:
            model, best_val, best_epoch = train_one(
                ...
                lr, dropout, device, cid, ci + 1, len(configs))
            corrs = evaluate(model, X_eval, y_eval_mean, device)
        except Exception as e:
            print(f"    [{ci+1}/{len(configs)}] {cid}  ERROR: {e}", flush=True)
            try:
                if torch.cuda.is_available():
                    torch.cuda.empty_cache()
            except Exception:
                pass  # GPU state may be unrecoverable; just continue
            # Log NaN row so we don't retry on resume
            row = {
                "config_id": cid, "window": window, "horizon": horizon,
                "lr": lr, "dropout": dropout, "n_steps": n_steps,
                "train_stride": train_stride, "eval_stride": eval_stride,
                "r_nano": float("nan"), "r_ns": float("nan"), "r_sm": float("nan"),
                "combined_score": float("nan"),
                "best_val_loss": float("nan"), "best_epoch": -1,
                "elapsed_s": round(time.time() - t0, 1),
            }
            # If CUDA is in a bad state, write the row and exit so the
            # process can be restarted (resume will skip completed rows).
            if "CUDA" in str(e) or "cuda" in str(e):
                _fsync_write(TSV_PATH, "a",
                             "\t".join(str(row[c]) for c in TSV_COLUMNS) + "\n")
                print(f"    CUDA error detected — exiting for restart. "
                      f"{cid} logged as NaN.", flush=True)
                sys.exit(2)
        else:
            combined = 0.5 * corrs["ns"] + 0.5 * corrs["sm"]
            elapsed = time.time() - t0
            print(f"    [{ci+1}/{len(configs)}] {cid:32s}  "
                  f"nano={corrs['nano']:.4f} ns={corrs['ns']:.4f} "
                  f"sm={corrs['sm']:.4f} comb={combined:.4f} "
                  f"ep{best_epoch} ({elapsed:.0f}s)", flush=True)
            row = {
                "config_id": cid, "window": window, "horizon": horizon,
                "lr": lr, "dropout": dropout, "n_steps": n_steps,
                "train_stride": train_stride, "eval_stride": eval_stride,
        ...
        # Append to TSV with flush+fsync so the row is durable on disk.
        _fsync_write(TSV_PATH, "a",
                     "\t".join(str(row[c]) for c in TSV_COLUMNS) + "\n")
        done_ids.add(cid)
    ...
    valid = all_df.dropna(subset=["combined_score"])

    print(f"\n{'='*90}")
    print(f"  TOP 10 by combined_score (0.5*r_ns + 0.5*r_sm)")
    print(f"{'='*90}")
    top10 = valid.sort_values("combined_score", ascending=False).head(10).reset_index(drop=True)
    print(f"  {'#':>3} {'config':32s} {'comb':>8s} {'r_ns':>8s} {'r_sm':>8s} {'r_nano':>8s}")
    print(f"  {'-'*76}")
    for i, r in top10.iterrows():
        print(f"  {i+1:>3d} {r['config_id']:32s} "
              f"{r['combined_score']:>8.4f} {r['r_ns']:>8.4f} "
              f"{r['r_sm']:>8.4f} {r['r_nano']:>8.4f}")

    print(f"\n{'='*90}")
    print(f"  TOP 5 by r_sm")
    print(f"{'='*90}")
    top5 = valid.sort_values("r_sm", ascending=False).head(5).reset_index(drop=True)
    print(f"  {'#':>3} {'config':32s} {'r_sm':>8s} {'r_ns':>8s} {'comb':>8s} {'r_nano':>8s}")
    print(f"  {'-'*76}")
    for i, r in top5.iterrows():
        print(f"  {i+1:>3d} {r['config_id']:32s} "
              f"{r['r_sm']:>8.4f} {r['r_ns']:>8.4f} "
              f"{r['combined_score']:>8.4f} {r['r_nano']:>8.4f}")

    elapsed_total = time.time() - t_start
    print(f"\n  Total sweep time: {elapsed_total:.1f}s "
