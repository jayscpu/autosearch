...
def main():
    ...
    # ── Normalize targets (fit on training windows only) ──
    # y shape: (N_windows, n_steps, N_TARGETS) → flatten to (N*steps, N_TARGETS)
    y_train_flat = y_train.reshape(-1, N_TARGETS)
    target_mean = y_train_flat.mean(axis=0)  # (N_TARGETS,)
    target_std = y_train_flat.std(axis=0)    # (N_TARGETS,)
    target_std[target_std < 1e-8] = 1.0      # safety

    print(f"  Target normalization (from training):", flush=True)
    for i, name in enumerate(target_cols):
        print(f"    {name:28s}: mean={target_mean[i]:.6f}  std={target_std[i]:.6f}",
              flush=True)
    ...
    y_earlystop = norm_y(y_earlystop)

    # Combine within + cross for evaluation (keep unnormalized for metrics)
    X_eval = np.concatenate([X_within, X_cross], axis=0)
    y_eval_raw = np.concatenate([y_within, y_cross], axis=0)
    seq_eval = seq_within + seq_cross

    y_eval_mean = y_eval_raw.mean(axis=1)  # (N, N_TARGETS) — original scale
    ...
    pred_normed = np.mean(all_preds, axis=0).mean(axis=1)  # (N, N_TARGETS)
    pred_mean = denorm_y(pred_normed)

    # ── Per-target MSE (original scale) ──
    ...
    out_path = SCRIPT_DIR / "predictions_recovery_clipped_normed.csv"
