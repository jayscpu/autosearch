#!/usr/bin/env python3
"""
Multi-output LSTM predicting miss_rate_nano + recovery deltas (WEIGHTED).

Same as train_recovery.py but uses inverse-variance weighted MSE
[0.22, 0.76, 2.02] for [miss_rate_nano, recovery_nano_to_small, recovery_small_to_medium]
to focus the optimizer on recovery predictions.

Outputs predictions_recovery_weighted.csv.
"""
...
def train_model(X_train, y_train, X_val, y_val, n_feat, n_steps, device):
    ...
    # Inverse-variance weights: [miss_rate_nano, recovery_ns, recovery_sm]
    target_weights = torch.tensor([0.22, 0.76, 2.02], device=device)

    def weighted_mse(pred, target):
        # pred/target shape: (batch, n_steps, n_targets)
        sq_err = (pred - target) ** 2  # (batch, n_steps, n_targets)
        return (sq_err * target_weights).mean()
    ...
    for epoch in range(CONFIG["max_epochs"]):
        ...
        model.eval()
        with torch.no_grad():
            val_loss = weighted_mse(model(val_x), val_y).item()
...
def main():
    ...
    out_path = SCRIPT_DIR / "predictions_recovery_weighted.csv"
